...
[ASP.NET Core MVC Model Binding](https://geeksarray.com/blog/aspnet-core-mvc-model-binding).

**Start here for this article:** `Controllers/DemoController.cs` — the `FromQuery`, `PostForm`, `PostBody`, and `RouteBind` actions each bind from a different request source; `Controllers/SuppliersController.cs` shows complex-type binding with validation.
